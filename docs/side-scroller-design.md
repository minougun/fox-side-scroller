...
- 2026-04-25 update: language and difficulty choices are now shown only before the first start in a browser session. Retry and all-clear restarts keep the selected setup and show only the restart action. The canvas display preserves the native 16:9 game ratio while the start controls are arranged in one stacked menu below the playfield to avoid overlap on small screens.
- 2026-04-25 update: enemy types now have attack affinities. Beetles resist stomps, slimes absorb gunshots, drones resist stomps and dash attacks, and pods deflect swords. Weak attacks create a small shield spark without damage, while preferred attacks add damage so enemy roles matter during combat.
- 2026-04-25 update: enemy score now charges a special gauge in the HUD. When the gauge reaches 100%, the game automatically enters a 9-second special mode with a screen glow, activation burst, countdown HUD, resistance-piercing attacks, extra damage, and double score on enemy defeats. Reduced-motion mode keeps the countdown and gameplay effects while limiting pulse/particle intensity.
