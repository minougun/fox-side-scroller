...
- 2026-04-25 update: language and difficulty choices are now shown only before the first start in a browser session. Retry and all-clear restarts keep the selected setup and show only the restart action. The canvas display now fills the viewport to reduce unused margins around the playfield.
